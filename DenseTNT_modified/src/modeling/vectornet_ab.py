...
class NewSubGraph(nn.Module):
    ...
    def __init__(self, hidden_size, depth=None):
        ...
        # self.processing_layer = MLP(2*hidden_size, hidden_size, 0.2)
    ...
    def forward(self, input_list: list, bev=None, map=False):
        ...
        if bev is not None:
            ...
            # hidden_states = self.processing_layer(torch.cat([hidden_states, bev], dim=-1))
            # bev_hidden = self.processing_layer(bev)
            # hidden_states = bev_hidden
            # hidden_states = hidden_states + bev_hidden
            # pass
            # if map == False:
            #     # bev_hidden = self.processing_layer(bev)
            #     # hidden_states[-bev_hidden.shape[0]:] = bev_hidden
                
            #     hidden_states[-bev.shape[0]:] = bev
            # elif map == True:
            #     # bev_hidden = self.processing_layer(bev)
            #     # hidden_states = bev_hidden
...
class VectorNet(nn.Module):
    ...
    def forward_encode_sub_graph(self, mapping: List[Dict], matrix: List[np.ndarray], polyline_spans: List[List[slice]],
        ...
        for i in range(batch_size):
            ...
            input_list_list.append(input_list)     # num_agents+num_lanes, 128    first 20 elemnts 
        ...
        if 'lane_scoring' in args.other_params:
            lane_states_batch = []
            for i in range(batch_size):
                # a, b = self.point_level_sub_graph(map_input_list_list[i], bev=batch_features[i], map=True)
        ...
        if 'laneGCN' in args.other_params:
            for i in range(batch_size):
                ...
                # concatenated_features = torch.cat([agents, batches[i]], dim=1)
                # output = self.processing_layer(concatenated_features)
                # output = agents + batches[i]
                # element_states_batch[i] = torch.cat([output, lanes])

                # element_states_batch[i] = torch.cat([batches[i], lanes])
                # breakpoint()
                # element_states_batch[i] = torch.cat([agents, lanes, batches[i]])
    ...
    def forward(self, mapping: List[Dict], device):
        ...
        # batches = torch.split(vit_features, num_agents, dim=0)
        # for i in range(batch_size):
        #     hidden_states[i][:mapping[i]['map_start_polyline_idx']] = self.processing_layer(torch.cat([hidden_states[i][:mapping[i]['map_start_polyline_idx']],
                                                                                            # batches[i]], dim = -1))
        
        # plot_trajectory(mapping[0]['agents'])
        # import pdb; pdb.set_trace()
        ...
        # lane_states_batch[0].shape = 81, 128; len(lane_states_batch) = batch_size
        # len(element_states_batch) = batch_size, element_states_batch[0].shape = 97, 128
        # inputs.shape = 1, 97, 128
        # input_lengths = [97]
        # hidden_states.shape = 1, 97, 128
        # len(mapping[0]['agents']) = 16, len(mapping[0]['polygons']) = 81, len(mapping[0]['polyline_spans']) = 97
        # mapping[0]['agents'] == mapping[0]['trajs'] are 16, 20, 2 
