...
class CenterPixelCrossAttention(nn.Module):
    ...
    def forward(self, x, patch_indices, scale):
        ...
        new_patch_indices = new_patch_indices.unsqueeze(1)   # 79, 1

        # selected_x = x[torch.arange(x.size(0)).unsqueeze(1), new_patch_indices, :]
        # breakpoint()
        # print(patch_indices)
        # print(new_patch_indices)
        selected_x = x.gather(1, new_patch_indices.unsqueeze(-1).expand(-1, -1, x.shape[2]))     # 79, 1, 128
        # breakpoint()
...
class Transformer(nn.Module):
    ...
    def forward(self, x, patch_indices, scale):
        ...
        for attn, ff in self.layers:
            ...
            attn_output = attn(x, patch_indices, scale)  # Shape: (339, 1, 128)
...
#         # Conv2d expects the input in the format (batch_size, channels, height, width)
#         # Since your last dimension is the channels, we'll adjust for that in the forward method.
#         # self.conv = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
#         self.linear = nn.Linear(200*100, 100*50)
#         self.apply(init_weights)

#     def forward(self, x):
#         agent_num = x.shape[0]
#         # Adjust the input shape to (batch_size, channels, height, width)
#         x = x.permute(0, 3, 1, 2)  # From (agent_number, 200, 100, 256) to (agent_number, 256, 200, 100)
...
#         # Reshape back to the desired shape (agent_number, 256, 100, 50)
#         x = x.view(agent_num, 256, 100, 50)
#         # print(x.shape)
#         # Adjust the output shape back to (batch_size, height, width, channels)
#         x = x.permute(0, 2, 3, 1)  # Back to (agent_number, 100, 50, 256)
#         return x
...
class ViT(nn.Module):
    ...
    def forward(self, data, conv=False):
        ...
        # extended_tensor = torch.cat((torch.tensor([0], dtype=num_agents.dtype), num_agents), dim=0)
        # av_index = torch.cumsum(extended_tensor, dim=0)
        # breakpoint()
        ...
        # breakpoint()
        x = self.transformer(x, patch_indices, self.scale)                          # 339, 100, 128
        # breakpoint()
...
def get_patch_idx(data, patch_height, patch_width, image_height, image_width):
    # coordinates = torch.stack([data_['agents'] for data_ in data]).cuda()
    ...
    # breakpoint()
    # mask_1 = torch.isclose(patch_indices, torch.tensor(v_patch), atol=0.001)
    # mask_2 = torch.isclose(patch_indices, torch.tensor(h_patch), atol=0.001)
    # mask_3 = (patch_indices == -1)
    # breakpoint()

    # patch_indices[mask_1] = v_patch - 1
    # patch_indices[mask_2] = h_patch - 1
    # patch_indices[mask_3] = 0
