...
class StreamMapNet(BaseMapper):
    ...
    def forward_train(self, img, vectors, points=None, img_metas=None, **kwargs):
        ...
        # breakpoint()
        gts, img, img_metas, valid_idx, points = self.batch_data(vectors, img, img_metas, img.device, points)
    ...
    def forward_test(self, img, points=None, img_metas=None, **kwargs):
        ...
        #  prepare labels and images
        with record_function("pre_bev_extraction"):
            tokens = []
            for img_meta in img_metas:
                tokens.append(img_meta['token'])

            _bev_feats = self.backbone(img, img_metas, points=points)    # 1, 256, 50, 100
            # breakpoint()
