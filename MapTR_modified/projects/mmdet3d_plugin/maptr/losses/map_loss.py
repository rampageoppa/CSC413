...
@mmcv.jit(derivate=True, coderize=True)
@weighted_loss
def pts_nll_loss_guassian(pred, target):
    sample_size = target.shape[0] * target.shape[1]  # number of 2-D points

    # Reshape predictions and targets
    pred_reshape = pred.reshape(sample_size, 5)      # [x_mean, y_mean, sigma_x, sigma_y, rho_xy]
    pts_pred = pred_reshape[:, :2]
    pts_variance = pred_reshape[:, 2:4]
    pts_rho = pred_reshape[:, 4:]
    target = target.reshape(sample_size, 2)

    # Create covariance matrix for each item in batch
    # sigma_x = pts_variance[:, 0] + 1e-5
    # sigma_y = pts_variance[:, 1] + 1e-5
    sigma_x = torch.clamp(pts_variance[:, 0], min=1e-5, max=10)
    sigma_y = torch.clamp(pts_variance[:, 1], min=1e-5, max=10)
    rho_xy = pts_rho[:, 0]

    L = torch.zeros(sample_size, 2, 2).to(pred.device)  
    L[:, 0, 0] = sigma_x 
    L[:, 0, 1] = 0
    L[:, 1, 0] = rho_xy * sigma_y
    # L[:, 1, 1] = torch.sqrt(sigma_y**2 - (rho_xy * sigma_y)**2)
    # L[:, 1, 1] = torch.sqrt(sigma_y**2 - (rho_xy * sigma_y)**2 + 1e-8)
    L[:, 1, 1] = torch.sqrt(torch.clamp(sigma_y**2 - (rho_xy * sigma_y)**2, min=1e-5)) 


    m = MultivariateNormal(pts_pred, scale_tril=L)
...
class PtsNLLLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss_bbox = self.loss_weight * pts_nll_loss_guassian(pred, target, weight, reduction=reduction, avg_factor=avg_factor)
        return loss_bbox
