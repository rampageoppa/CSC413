...
class new_reg_branch_guassian(nn.Module):
...
class MapTRHead(DETRHead):
    ...
    def _init_layers(self):
        ...
        reg_branch = new_reg_branch_guassian()
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        for lvl in range(hs.shape[0]):
            ...
            reg_out = self.reg_branches[lvl](hs[lvl])    # 4, 1000, 5
            # breakpoint()
            tmp = reg_out[:, :, 0:2]
            variance = reg_out[:, :, 2:5]

            outputs_pts_variance = variance.view(variance.shape[0], self.num_vec, self.num_pts_per_gt_vec, 3) # 4, 50, 20, 3
            # breakpoint()
            ...
            outputs_pts_variances.append(outputs_pts_variance)

        outputs_classes = torch.stack(outputs_classes)
        # breakpoint()
        outputs_coords = torch.stack(outputs_coords)
        outputs_pts_coords = torch.stack(outputs_pts_coords)
        outputs_pts_variances = torch.stack(outputs_pts_variances)    # 6, 4, 50, 20, 3
        # breakpoint()
        outs = {
            'bev_embed': bev_embed,
            ...
            'all_pts_variances': outputs_pts_variances,
            'enc_cls_scores': None,
            'enc_bbox_preds': None,
            'enc_pts_preds': None
        }
    ...
    def loss_single(self,
        ...
        pts_variances_list = [pts_variances[i] for i in range(num_imgs)]
        # import pdb;pdb.set_trace()
        ...
        pts_variances = pts_variances.reshape(-1, pts_variances.size(-2), pts_variances.size(-1))
        out = torch.cat([pts_preds, pts_variances], dim=2)
        # breakpoint()
        if self.num_pts_per_vec != self.num_pts_per_gt_vec:
            pts_preds = pts_preds.permute(0,2,1)
            pts_preds = F.interpolate(pts_preds, size=(self.num_pts_per_gt_vec), mode='linear',
    ...
    def loss(self,
        ...
        all_pts_variances = preds_dicts['all_pts_variances']
        ...
        losses_cls, losses_bbox, losses_iou, losses_pts, losses_dir = multi_apply(
            self.loss_single, all_cls_scores, all_bbox_preds, all_pts_preds, all_pts_variances,
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            labels = preds['labels']
            pts = preds['pts']
            variances = preds['variances']

            bev = bev_embed[:, i, :]
            raw_scores = preds['raw_scores']

            ret_list.append([bboxes, scores, labels, pts, variances, bev, raw_scores])
