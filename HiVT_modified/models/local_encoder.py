...
from .mod_vit_encoder import ViT
...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # self.al_encoder = ALEncoder(node_dim=4, #2
        #                             edge_dim=4, #2
        #                             embed_dim=embed_dim,
        #                             num_heads=num_heads,
        #                             dropout=dropout)
        ...
        #                         heads = 16,
        #                         mlp_dim = 512,
        #                         dropout = 0.1, #0.1 (ab_good)
        #                         emb_dropout = 0.1)
        # StreamMapNet
        # self.vit_encoder = ViT(image_size = (100, 50),
        #                         patch_size = (10, 5),
        #                         num_classes = embed_dim,  # 128 
        #                         pool = 'mean',
        #                         channels = 256,  #512
        #                         dim = 128,  # 128 
        #                         depth = 6,  # 6
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        # with record_function("local_forward"):
        if True:
            vit_embed = self.vit_encoder(data.clone())

            for t in range(self.historical_steps):
                # torch.cuda.set_device('cpu')  # Force PyTorch to use the CPU
            ...
            out = self.temporal_encoder(x=out, padding_mask=data['padding_mask'][:, : self.historical_steps])       # 339, 128
            ...
            # breakpoint()
            # out = self.al_encoder(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr, rotate_mat=data['rotate_mat'])  # 339, 128
            # return out
...
class ALEncoder(MessagePassing):
    ...
    def message(self,
            ...
            if rotate_mat is None:
                ...
                # var_x = 2*(x_j[:,2]**2)
                # var_y = 2*(x_j[:,3]**2)
                # std_x = torch.sqrt(var_x*cos**2 + var_y*sin**2).unsqueeze(dim=1)
                # std_y = torch.sqrt(var_x*sin**2 + var_y*cos**2).unsqueeze(dim=1)

                # var_x_edge = 2*(edge_attr[:,2]**2)
                # var_y_edge = 2*(edge_attr[:,3]**2)
                # std_x_edge = torch.sqrt(var_x_edge*cos**2 + var_y_edge*sin**2).unsqueeze(dim=1)
                # std_y_edge = torch.sqrt(var_x_edge*sin**2 + var_y_edge*cos**2).unsqueeze(dim=1)
                # x_j = self.lane_embed([torch.cat([torch.bmm(x_j[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), torch.cat([std_x, std_y], dim=-1)],dim=-1), 
                                        # torch.cat([torch.bmm(edge_attr[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), torch.cat([std_x_edge, std_y_edge], dim=-1)],dim=-1)])
                
                # x_j = self.lane_embed([torch.cat([torch.bmm(x_j[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), x_j[:,2:4]],dim=-1), 
                #                         torch.cat([torch.bmm(edge_attr[:,:2].unsqueeze(-2), rotate_mat).squeeze(-2), edge_attr[:,2:4]],dim=-1)])

                # rotate_x_j = torch.bmm(torch.stack((x_j[:,0],x_j[:, 2]), dim=1).unsqueeze(-2), rotate_mat).squeeze(-2)
                # rotate_edge_attr = torch.bmm(torch.stack((edge_attr[:,0], edge_attr[:, 2]), dim=1).unsqueeze(-2), rotate_mat).squeeze(-2)
                # torch.stack((rotate_x_j[:, 0], x_j[:, 1], rotate_x_j[:, 1], x_j[:, 3]), dim=1)
                # x_j = self.lane_embed([torch.stack((rotate_x_j[:, 0], x_j[:, 1], rotate_x_j[:, 1], x_j[:, 3]), dim=1), 
                #                         torch.stack((rotate_edge_attr[:, 0], edge_attr[:, 1], rotate_edge_attr[:, 1], edge_attr[:, 3]), dim=1)])
            query = self.lin_q(x_i).view(-1, self.num_heads, self.embed_dim // self.num_heads)
            key = self.lin_k(x_j).view(-1, self.num_heads, self.embed_dim // self.num_heads)
            value = self.lin_v(x_j).view(-1, self.num_heads, self.embed_dim // self.num_heads)
            scale = (self.embed_dim // self.num_heads) ** 0.5
    ...
    def _mha_block_2(self,
                   ...
                   size: Size) -> torch.Tensor:
        # breakpoint()
        # torch.cat([x_lane, betas]) torch.cat([x_lane, betas], dim=-1)
        x_actor = self.out_proj(self.propagate(edge_index=edge_index, x=(x_lane, x_actor), edge_attr=edge_attr, 
