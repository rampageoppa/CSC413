...
class HiVT(pl.LightningModule):
    ...
    def forward(self, data: TemporalData):
        ...
        local_embed = self.local_encoder(data=data)    # num_agents, 128

        global_embed = self.global_interactor(data=data, local_embed=local_embed)     # 6, num_agents, 128
        y_hat, pi = self.decoder(local_embed=local_embed, global_embed=global_embed)
        return y_hat, pi

    def training_step(self, data, batch_idx):
        y_hat, pi = self(data)                                            # 6, 737, 30, 4; 737, 6
        reg_mask = ~data['padding_mask'][:, self.historical_steps:]       # 737, 30
        valid_steps = reg_mask.sum(dim=-1)                                # 737
        cls_mask = valid_steps > 0                                        # 737
        l2_norm = (torch.norm(y_hat[:, :, :, : 2] - data.y, p=2, dim=-1) * reg_mask).sum(dim=-1)  # [F, N] 6, 737
        best_mode = l2_norm.argmin(dim=0)                                 # 737
        y_hat_best = y_hat[best_mode, torch.arange(data.num_nodes)]       # 737, 30, 4
        reg_loss = self.reg_loss(y_hat_best[reg_mask], data.y[reg_mask])
        soft_target = F.softmax(-l2_norm[:, cls_mask] / valid_steps[cls_mask], dim=0).t().detach()      # 495, 6
    ...
    def save_prof(self, prof, data):
        save_path = '/home/guxunjia/project/HiVT_modified/runtime_result_hivt.pkl'
        if os.path.exists(save_path):
            with open(save_path, 'rb') as file:
                profiling_results = pickle.load(file)
        else:
            profiling_results = {}

        # Extract the specific times for 'hivt_forward' and 'local_forward'
        for avg in prof.key_averages():
            if avg.key == "hivt_forward" or avg.key == "local_forward":
                # breakpoint()
                if data['seq_id'][0].item() not in profiling_results:
                    profiling_results[data['seq_id'][0].item()] = {}

                profiling_results[data['seq_id'][0].item()][avg.key] = {
                    "cpu_time": avg.cpu_time_total,  # Total CPU time in microseconds
                    "cuda_time": avg.cuda_time_total  # Total CUDA time in microseconds
                }
    
        # Save the profiling results to a pickle file
        with open(save_path, 'wb') as f:
            pickle.dump(profiling_results, f)

    def validation_step(self, data, batch_idx):
        # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
            # with record_function("hivt_forward"):
        y_hat, pi = self(data)
        
        # self.save_prof(prof, data)
        
        # breakpoint()
        # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
        # for avg in prof.key_averages():
        #     if "forward" in avg.key:
        #         print(f"{avg.key:25} {avg.cpu_time_total:.3f}ms")
        #         print(f"{avg.key:25} {avg.cuda_time_total:.3f}ms")
    ...
    def visualization(self, data, batch_idx):
        ...
        save_path = '/home/guxunjia/project/HiVT_modified/result.pkl'
        if os.path.exists(save_path):
            with open(save_path, 'rb') as file:
    ...
    def visualization_all(self, data, batch_idx):
        ...
        for i in range(data['positions'].shape[0]):
            # predict_data[data['seq_id'][i].item()] = y_hat_agent[i].cpu().numpy()
            # predict_data[data['seq_id'][i].item()] = torch.cat([y_hat_agent[i], y_hat_agent_uncertainty[i]], dim=-1).cpu().numpy()
            if i == data['positions'].shape[0] - 1:
                traj_list.append(torch.cat([y_hat_agent[i], y_hat_agent_uncertainty[i]], dim=-1).cpu())
                predict_data[data['seq_id'][index].item()] = torch.cat([v.unsqueeze(0) for v in traj_list], dim=0).numpy()
                break

            new_index = (cumsum_count > i).nonzero().min().item()
            if new_index > index:
                predict_data[data['seq_id'][index].item()] = torch.cat([v.unsqueeze(0) for v in traj_list], dim=0).numpy()
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--node_dim', type=int, default=2) #2
        parser.add_argument('--edge_dim', type=int, default=2) #2
        parser.add_argument('--embed_dim', type=int, required=True)
        parser.add_argument('--num_heads', type=int, default=8)
        parser.add_argument('--dropout', type=float, default=0.2) #0.1 0.2
        parser.add_argument('--num_temporal_layers', type=int, default=4)
        parser.add_argument('--num_global_layers', type=int, default=3)
        parser.add_argument('--local_radius', type=float, default=50)
        parser.add_argument('--parallel', type=bool, default=False)
        parser.add_argument('--lr', type=float, default=3.5e-4)  #5e-4
