...
folder_path = "/home/guxunjia/project/HiVT_data/maptrv2_cent/val/data/"
data = {}
...
# with open('/home/guxunjia/project/HiVT_modified/result_maptr_mini_all.pkl', 'rb') as handle:
with open('/home/guxunjia/project/HiVT_modified/result_maptr_full_all.pkl', 'rb') as handle:
    predict_data = pickle.load(handle)

with open('/home/guxunjia/project/HiVT_modified/result_maptr_std_full_all.pkl', 'rb') as handle:
    predict_data_std = pickle.load(handle)

for key, value in predict_data.items():
    try:
        data[key]['predict_fut'] = value
        data[key]['predict_fut_std'] = predict_data_std[key]
    except:
        breakpoint()

for scene_index, (key, value) in enumerate(tqdm(data.items())):
    if scene_index < 1280: #1279
        continue
    else:
        pass
    ...
    # plt.show()
    plt.savefig('/home/guxunjia/project/HiVT_modified/plots/maptr_full_all/' + value['sample_token'] + '.png', bbox_inches='tight', format='png',dpi=600) #dpi=1200
