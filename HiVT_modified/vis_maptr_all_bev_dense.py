...
# folder_path = "/home/guxunjia/project/HiVT_data/stream/full_val/data/"
...
# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/maptr_al.pkl', 'rb') as handle:
# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/stream_al.pkl', 'rb') as handle:
with open('/home/guxunjia/project/DenseTNT_modified/bev_results/full/maptr_al.pkl', 'rb') as handle:
    predict_data = pickle.load(handle)

# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/maptr_al_unc.pkl', 'rb') as handle:
# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/stream_al_unc.pkl', 'rb') as handle:
with open('/home/guxunjia/project/DenseTNT_modified/bev_results/full/maptr_al_unc.pkl', 'rb') as handle:
    predict_data_std = pickle.load(handle)

# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/maptr_ab.pkl', 'rb') as handle:
# with open('/home/guxunjia/project/HiVT_modified/bev_results/full/stream_ab.pkl', 'rb') as handle:
with open('/home/guxunjia/project/DenseTNT_modified/bev_results/full/maptr_ab.pkl', 'rb') as handle:
...
for scene_index, (key, value) in enumerate(tqdm(data.items())):
    ...
    if divider.size != 0:
        ...
        plot_points_with_laplace_variances(n_divider[:,:,0], n_divider[:,:,1], divider_betas[:,:,0], divider_betas[:,:,1], 'orange', value['sample_token'], ax[0], heading)
        ...
        plot_points_with_laplace_variances(n_divider[:,:,0], n_divider[:,:,1], divider_betas[:,:,0], divider_betas[:,:,1], 'orange', value['sample_token'], ax[2], heading)
    if ped_crossing.size != 0:
        ...
        plot_points_with_laplace_variances(n_ped_crossing[:,:,0], n_ped_crossing[:,:,1], ped_crossing_betas[:,:,0], ped_crossing_betas[:,:,1], 'blue', value['sample_token'], ax[0], heading)
        ...
        plot_points_with_laplace_variances(n_ped_crossing[:,:,0], n_ped_crossing[:,:,1], ped_crossing_betas[:,:,0], ped_crossing_betas[:,:,1], 'blue', value['sample_token'], ax[2], heading)
    if boundary.size != 0:
        n_boundary = np.array(normalize_lanes(x, y, boundary, heading))
        n_boundary = n_boundary[:, :, [1, 0]]
        n_boundary[:,:,0] = -n_boundary[:,:,0]
        plot_points_with_laplace_variances(n_boundary[:,:,0], n_boundary[:,:,1], boundary_betas[:,:,0], boundary_betas[:,:,1], 'green', value['sample_token'], ax[0], heading)
        ...
        plot_points_with_laplace_variances(n_boundary[:,:,0], n_boundary[:,:,1], boundary_betas[:,:,0], boundary_betas[:,:,1], 'green', value['sample_token'], ax[2], heading)
    ...
    ax[0].plot(n_ego_hist[:, 1], n_ego_hist[:, 0], c='grey')
    ax[0].plot(n_ego_fut[:, 1], n_ego_fut[:, 0], c='red')
    ax[1].plot(n_ego_hist[:, 1], n_ego_hist[:, 0], c='grey')
    ax[1].plot(n_ego_fut[:, 1], n_ego_fut[:, 0], c='red')
    ax[2].plot(n_ego_hist[:, 1], n_ego_hist[:, 0], c='grey')
    ...
    if keep_indices == []:
        ...
        for agent_index in keep_indices:
            ...
            ax[0].plot(n_agent_hist[:, 1], n_agent_hist[:, 0], c='grey')
            ax[0].plot(n_agent_fut[:, 1], n_agent_fut[:, 0], c='red')
            ax[1].plot(n_agent_hist[:, 1], n_agent_hist[:, 0], c='grey')
            ax[1].plot(n_agent_fut[:, 1], n_agent_fut[:, 0], c='red')
            ax[2].plot(n_agent_hist[:, 1], n_agent_hist[:, 0], c='grey')
    ...
    for i in range(len(value['predict_fut'])):
        ...
        for j in range(6):
            ...
            ax[0].plot(n_predict_fut[:, 0], n_predict_fut[:, 1], c='pink')
    ...
    for i in range(len(value['predict_fut_std'])):
        ...
        for j in range(6):
            ...
            ax[1].plot(n_predict_fut[:, 0], n_predict_fut[:, 1], c='pink')
    ...
    for i in range(len(value['predict_fut_bev'])):
        ...
        for j in range(6):
            ...
            ax[2].plot(n_predict_fut[:, 0], n_predict_fut[:, 1], c='pink')

    # if value['sample_token'] == '61a7bd24f88a46c2963280d8b13ac675':

    plt.show()
    # plt.savefig('/home/guxunjia/project/DenseTNT_modified/bev_plots/full/maptr/' + value['sample_token'] + '.png', bbox_inches='tight', format='png',dpi=100) #dpi=1200
