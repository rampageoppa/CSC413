...
def process_nuscenes(split, raw_path):
    ...
    # breakpoint()
    (lane_vectors, lane_actor_index, lane_actor_vectors) = get_lane_features(np.arange(0, all_node_pos.shape[0]), node_positions_19, origin, rotate_mat, data['vec_map']['centerlines'], data['vec_map']['centerline_betas'])
    # breakpoint()
    # (lane_vectors, lane_actor_index, lane_actor_vectors) = get_lane_features(np.arange(0, all_node_pos.shape[0]), node_positions_19, origin, rotate_mat, data['predicted_map']['centerline'], data['predicted_map']['centerline_betas'])
    ...
    bev_embed = torch.tensor(data['vec_map']['bev_features'].reshape(200, 100, 256))
...
def get_lane_features(node_inds,
    ...
    # node_positions = torch.stack((node_positions[:, 0], torch.zeros_like(node_positions[:, 0]), node_positions[:, 1], torch.zeros_like(node_positions[:, 1])), dim=1)
    ...
    try:
        ...
        # lane_positions = torch.stack((lane_positions[:, 0], lane_position_betas[:, 0], lane_positions[:, 1], lane_position_betas[:, 1]), dim=1)
    except:
        breakpoint()
    lane_vectors = torch.cat(lane_vectors, dim=0)
    lane_vector_betas = torch.cat(lane_vector_betas, dim=0)
    # lane_vectors = torch.cat((lane_vectors, lane_vector_betas), dim=-1)
    lane_vectors = torch.stack((lane_vectors[:, 0], lane_vector_betas[:, 0], lane_vectors[:, 1], lane_vector_betas[:, 1]), dim=1)
