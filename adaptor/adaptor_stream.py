...
def main():
    ...
    for i in tqdm(range(len(stream_data)), desc="Merging MapTR Prediction"):
        predicted_map = {}
        sample_token = stream_data[i]['token']
        try:
            idx = sample_to_idx[sample_token]
        except:
            continue

        stream_data[i]['vectors'], stream_data[i]['betas'] = scale_stream(stream_data[i])
        divider_index = np.where(stream_data[i]['labels'] == 1)
        ped_crossing_index = np.where(stream_data[i]['labels'] == 0)
        boundary_index = np.where(stream_data[i]['labels'] == 2)

        predict_divider = stream_data[i]['vectors'][divider_index]
        predict_ped_crossing = stream_data[i]['vectors'][ped_crossing_index]
        predict_boundary = stream_data[i]['vectors'][boundary_index]
        ...
        predict_divider_betas = stream_data[i]['betas'][divider_index]
        predict_ped_crossing_betas = stream_data[i]['betas'][ped_crossing_index]
        predict_boundary_betas = stream_data[i]['betas'][boundary_index]

        predict_divider_scores = stream_data[i]['scores'][divider_index]
        predict_ped_crossing_scores = stream_data[i]['scores'][ped_crossing_index]
        predict_boundary_scores = stream_data[i]['scores'][boundary_index]
